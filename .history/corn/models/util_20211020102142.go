...
func MakeStrategy(u User) (*[]Grid, error) {
	...
	if symbol.Category == "OKex" && u.Future == 1 {
		fmt.Println(currentGrid, "当前", symbol.MinTotal.Cmp(currentGrid.AmountBuy), "最小", symbol.MinTotal, symbol.MinAmount)
		...
		fmt.Println(currentGrid.AmountBuy, currentGrid.Mesure)
	} else if u.Future == 2 {
	...
	for _, g := range grids {
		...
		log.Printf("id:%2d - - moeny:%s - - 价格: %s - - 买入量: %s 张数:%v - - 卖出:%s--跌幅: %f",
			g.Id, g.TotalBuy.StringFixed(symbol.AmountPrecision+symbol.PricePrecision),
			g.Price.StringFixed(symbol.PricePrecision),
			g.AmountBuy.StringFixed(symbol.AmountPrecision),g.Mesure g.AmountSell.StringFixed(symbol.AmountPrecision), g.Decline)
		if u.Base < g.Id {

...
func MakeStrategy(u User) (*[]Grid, error) {
	...
	for i := 2; i <= int(u.Number); i++ {
		...
		if symbol.Category == "OKex" && u.Future == 1 {
			currentGrid.AmountBuy = currentGrid.TotalBuy.Div(currentPrice)
			fmt.Println(currentGrid.Id, currentGrid.AmountBuy, currentGrid.TotalBuy)
			if symbol.MinTotal.Cmp(currentGrid.AmountBuy) != -1 {
				currentGrid.AmountBuy = symbol.MinTotal
				currentGrid.Mesure = symbol.MinAmount
			} else {
				currentGrid.Mesure = currentGrid.AmountBuy.Mul(symbol.MinAmount.Div(symbol.MinTotal))
				currentGrid.AmountBuy = currentGrid.Mesure.Mul(symbol.MinTotal)
			}
		} else if u.Future == 2 {
			currentGrid.Mesure = currentGrid.AmountBuy
	...
	for _, g := range grids {
		log.Printf("id:%2d - - moeny:%s - - 价格: %s - - 买入量: %v 张数:%v - - 卖出:%s--跌幅: %f",
			g.Id, g.TotalBuy.StringFixed(symbol.AmountPrecision+symbol.PricePrecision),
			g.Price.StringFixed(symbol.PricePrecision),
			g.AmountBuy, g.Mesure, g.AmountSell.StringFixed(symbol.AmountPrecision), g.Decline)
		if u.Base < g.Id {
